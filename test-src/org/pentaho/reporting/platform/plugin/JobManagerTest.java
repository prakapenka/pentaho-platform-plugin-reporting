...
public class JobManagerTest extends JaxRsServerProvider {
  ...
  @Test public void testAddJobCorrectContentUUID() {
    client.path( String.format( URL_FORMAT, UUID.randomUUID().toString(), "/content" ) );
    Response response = client.post( null );
    assertNotNull( response );
    assertEquals( "There is no running tasks now!", 404, response.getStatus() );
  }
  ...
  @BeforeClass public static void initialize() throws Exception {
    ...
    PentahoSessionHolder.setStrategyName( JunitSessionStrategy.class.getName() );
    session = mock( IPentahoSession.class );
    when( session.getId() ).thenReturn( uuid.toString() );
  }
  ...
  public static class JunitSessionStrategy implements IPentahoSessionHolderStrategy {
    IPentahoSession iPentahoSession;

    @Override public void setSession( IPentahoSession iPentahoSession ) {
      this.iPentahoSession = iPentahoSession;
    }

    @Override public IPentahoSession getSession() {
      return session;
    }

    @Override public void removeSession() {
      this.iPentahoSession = null;
    }
  }
}
